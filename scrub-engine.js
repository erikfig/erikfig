...
function mountScrollWorld(container, config) {
  ...
  if (config.brand) {
    ...
    if (config.brand.logo) {
      const im = el('img', 'sw-brand__logo');
      im.src = config.brand.logo;
      im.alt = config.brand.name || '';
  ...
  function jumpTo(i) {
    ...
    const target = i === 0 ? seg.start
                 : i === N - 1 ? seg.end - window.innerHeight * 0.5
                 : seg.start + (seg.end - seg.start) * 0.5;
    if (reduce) { window.scrollTo({ top: target, behavior: 'auto' }); return; }
